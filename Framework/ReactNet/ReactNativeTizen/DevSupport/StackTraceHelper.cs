...
namespace ReactNative.DevSupport
{
    ...
    {
        public static IStackFrame[] ConvertJSCoreStackTrace(string stackTrace)
        {
            var lines = stackTrace.Split('\n');
            var frames = new IStackFrame[lines.Length - 1];
            for (var i = 1; i < lines.Length; ++i)
            {
                frames[i - 1] = new JSCoreStackFrame(lines[i]);
            }
        ...
        class JSCoreStackFrame : StackFrameBase
        {

            public JSCoreStackFrame(string line)
            {
                    FileName = line;
            }
