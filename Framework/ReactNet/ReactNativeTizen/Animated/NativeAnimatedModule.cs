...
namespace ReactNative.Animated
{
    ...
    public class NativeAnimatedModule : ReactContextNativeModuleBase, IOnBatchCompleteListener, ILifecycleEventListener
    {
        ...
        */
        ...
        UIManagerModule _uiMgrModule;
        NativeAnimatedNodesManager _nativeAnimnodeMgr;
        // END
        ...
        {
            Log.Info(ReactConstants.Tag, "## OnBatchComplete ->_operations.Count=" + _operations.Count);
        ...
        private bool OnEcoreRender()
        {
            //Log.Error(ReactConstants.Tag, ">>>>>>>>>>>>> OnEcoreRender-1 <<<<<<<<<<<<<<<<<<<");
            try
            {
                var operations = default(List<Action<NativeAnimatedNodesManager>>);
                lock (_operationsGate)
                ...
                if (operations != null)
                {
                    Log.Info(ReactConstants.Tag, ">>>>>>>>>>>>> OnEcoreRender-2 <<<<<<<<<<<<<<<<<<<");
                    foreach (var operation in operations)
                    {
                        operation(_nativeAnimnodeMgr);
                    }
                }

                if (_nativeAnimnodeMgr.HasActiveAnimations)
                {
                    Log.Info(ReactConstants.Tag, "### HasActiveAnimations sw.ElapsedMilliseconds = " + sw.ElapsedMilliseconds);

                    //TimeSpan timeSpan = new TimeSpan(60);
                    // 
                    _nativeAnimnodeMgr.RunUpdates(sw.Elapsed);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # createAnimatedNode ->tag=" + tag);
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # startListeningToAnimatedNodeValue ->tag=" + tag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # stopListeningToAnimatedNodeValue ->tag=" + tag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # dropAnimatedNode -> tag=" + tag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # setAnimatedNodeValue -> tag=" + tag + ", value=" + value);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # startAnimatingNode -> animationId=" + animationId + ", animatedNodeTag=" + animatedNodeTag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # stopAnimation -> animationId=" + animationId);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # connectAnimatedNodes -> parentNodeTag=" + parentNodeTag + ", childNodeTag=" + childNodeTag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # disconnectAnimatedNodes -> parentNodeTag=" + parentNodeTag + ", childNodeTag=" + childNodeTag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # connectAnimatedNodeToView -> animatedNodeTag=" + animatedNodeTag + ", viewTag=" + viewTag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # disconnectAnimatedNodeFromView -> animatedNodeTag=" + animatedNodeTag + ", viewTag=" + viewTag);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # addAnimatedEventToView -> viewTag=" + viewTag + ", eventName=" + eventName);
            _operations.Add(manager =>
        ...
        {
            Log.Info(ReactConstants.Tag, "# enqueue # removeAnimatedEventFromView -> viewTag=" + viewTag + ", eventName=" + eventName);
            _operations.Add(manager =>
