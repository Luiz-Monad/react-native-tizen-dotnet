...
namespace ReactNative.UIManager
{
    ...
    public class UIImplementation
    {
            ...
            int[] removeFrom)
        {
            ...
            {
                Log.Fatal(ReactConstants.Tag, "## ## addChildTags?.Length=" + addChildTags?.Length + " addAtIndexes?.Length=" + addAtIndexes?.Length);
                /*
                throw new ArgumentException(
            ...
            if (numToMove > 0)
            {
                for (var i = 0; i < numToMove; ++i)
                {
                    ...
                    Log.Info(ReactConstants.Tag, "[Move] child tag:"+ tagToMove + " to parent:"+ viewTag);
                }
            }

            if (numToAdd > 0)
            {
                if (addAtIndexes == null)
                {
                    throw new InvalidOperationException("addAtIndexes is null");
                }
                for (var i = 0; i < numToAdd; ++i)
                {
                    viewsToAdd[numToMove + i] = new ViewAtIndex(addChildTags[i], addAtIndexes[i]);

                    Log.Info(ReactConstants.Tag, "[Add] child tag:" + addChildTags[i] + " to parent:" + viewTag);
                }
            }

            if (numToRemove > 0)
            {
                Log.Fatal(ReactConstants.Tag, "## numToRemove=" + numToRemove);
                for (var i = 0; i < numToRemove; ++i)
                {
                    ...
                    Log.Info(ReactConstants.Tag, "[Remove] child tag:" + tagToRemove + " from parent:" + viewTag);
                }
            }
            ...
            for (var i = indicesToRemove.Length - 1; i >= 0; --i)
            {
                ...
                Log.Info(ReactConstants.Tag, ">>> indexToRemove=" + indexToRemove);
        ...
        private ReactShadowNode CreateRootShadowNode()
        {
            ...
            {
                Log.Info(ReactConstants.Tag, "### I18NUtil.IsRightToLeft is true ###");
        ...
        private void AssertNodeDoesNotNeedCustomLayoutForChildren(ReactShadowNode node)
        {
            ...
            // Check whether 'viewManager' own container props
        ...
        {
            using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "ReactShadowNode.CalculateLayout")
