...
namespace ReactNative.Animated
{
    class SpringAnimationDriver : AnimationDriver
    {
            ...
            : base(id, animatedValue, endCallback)
        {
            ...
            Log.Info(ReactConstants.Tag, "## _springFriction = " + _springFriction + ", _springTension=" + _springTension
                + ", Velocity="+ _currentState.Velocity + "_endValue="+ _endValue+ ", _restSpeedThreshold="+ _restSpeedThreshold
                + ", _displacementFromRestThreshold="+ _displacementFromRestThreshold+ ", _overshootClampingEnabled="+ _overshootClampingEnabled);
        }

        public override void RunAnimationStep(TimeSpan renderingTime)
        {
            //Log.Error(ReactConstants.Tag, "###  RunAnimationStep -> HasFinished = " + IsAtRest() + " _springStarted" + _springStarted);
            ...
            //Log.Error(ReactConstants.Tag, "###  RunAnimationStep -> AnimatedValue.Value = " + AnimatedValue.Value);
        ...
        private void Advance(double realDeltaTime)
        {
            if (IsAtRest())
            {
                Log.Error(ReactConstants.Tag, "### IsAtRest() return true ###");
                return;
            }
            ...
            //Log.Error(ReactConstants.Tag, "@@@@@@@@@@@@  adjustedDeltaTime= " + adjustedDeltaTime + ", realDeltaTime=" + realDeltaTime);
            ...
            //Log.Error(ReactConstants.Tag, "### _timeAccumulator = " + _timeAccumulator + ", SolverTimestepSec = " + SolverTimestepSec);
            ...
            if (IsAtRest() || (_overshootClampingEnabled && IsOvershooting()))
            {
                ...
                if (tension > 0)
                {
                    ...
                    //Log.Error(ReactConstants.Tag, "### _currentState.Position = " + _currentState.Position);
                }
                else
                {
