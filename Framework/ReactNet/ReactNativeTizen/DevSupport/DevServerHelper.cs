...
namespace ReactNative.DevSupport
{
    ...
    sealed class DevServerHelper : IDisposable
    {
        ...
        private const string BundleUrlFormat = "http://{0}/{1}.bundle?platform=tizen&dev={2}&hot={3}";
        private const string SourceMapUrlFormat = "http://{0}/{1}.map?platform=tizen&dev={2}&hot={3}";
        ...
        public async Task DownloadBundleFromUrlAsync(string jsModulePath, Stream outputStream, CancellationToken token)
        {
            ...
            Log.Info(ReactConstants.Tag, "DownloadBundleFromUrlAsync:Url= " + bundleUrl);
